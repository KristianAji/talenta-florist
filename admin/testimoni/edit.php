...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare('UPDATE testimoni SET nama=?,pesan=?,rating=?,aktif=? WHERE id=?')
             ->execute([$nama, $pesan, $rating, $aktif, $id]);
  ...
  <div class="admin-body">
    ...
    <div class="panel panel-form">
      ...
      <form method="post">
        ...
        <div class="form-group">
          <label>Pesan *</label>
          <textarea name="pesan" required><?= htmlspecialchars($t['pesan']) ?></textarea>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select name="rating">
              <?php for ($r = 5; $r >= 1; $r--): ?>
              <option value="<?= $r ?>" <?= $t['rating'] == $r ? 'selected' : '' ?>>
                <?= str_repeat('★', $r) ?> (<?= $r ?>)
              </option>
              <?php endfor; ?>
            </select>
          ...
          <div class="form-group form-check-inline">
            <label class="form-check">
              <input type="checkbox" name="aktif" value="1" <?= $t['aktif'] ? 'checked' : '' ?> />
              Tampilkan
            </label>
          </div>
        </div>
