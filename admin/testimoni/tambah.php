...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare('INSERT INTO testimoni (nama,pesan,rating,aktif) VALUES (?,?,?,?)')
             ->execute([$nama, $pesan, $rating, $aktif]);
    ...
    <div class="panel panel-form">
      ...
      <form method="post">
        ...
        <div class="form-group">
          ...
          <textarea name="pesan" required><?= htmlspecialchars($_POST['pesan'] ?? '') ?></textarea>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select name="rating">
              <?php for ($r = 5; $r >= 1; $r--): ?>
              <option value="<?= $r ?>" <?= (($_POST['rating'] ?? 5) == $r) ? 'selected' : '' ?>>
                <?= str_repeat('★', $r) ?> (<?= $r ?>)
              </option>
              <?php endfor; ?>
            </select>
