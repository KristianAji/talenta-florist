...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare('INSERT INTO testimoni (nama,pesan,rating,aktif) VALUES (?,?,?,?)')->execute([$nama,$pesan,$rating,$aktif]);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Testimoni berhasil ditambahkan.'];
...
<div class="admin-main">
  <div class="admin-topbar">
    ...
    <h1 class="topbar-title">Tambah <em>Testimoni</em></h1>
  ...
  <div class="admin-body">
    ...
    <div class="panel" style="max-width:600px;">
      <div class="panel-title">Form Testimoni</div>
      <form method="post">
        <div class="form-group"><label>Nama Pelanggan *</label>
          <input type="text" name="nama" value="<?= htmlspecialchars($_POST['nama']??'') ?>" required /></div>
        <div class="form-group"><label>Pesan / Ulasan *</label>
          <textarea name="pesan" required><?= htmlspecialchars($_POST['pesan']??'') ?></textarea></div>
        <div class="form-row">
          <div class="form-group"><label>Rating (1–5)</label>
            <select name="rating">
              <?php for ($r=5;$r>=1;$r--): ?>
              <option value="<?= $r ?>" <?= (($_POST['rating']??5)==$r)?'selected':'' ?>><?= str_repeat('★',$r) ?> (<?= $r ?>)</option>
              <?php endfor; ?>
            </select></div>
          <div class="form-group" style="display:flex;align-items:center;padding-top:1.6rem;">
            <label class="form-check">
              <input type="checkbox" name="aktif" value="1" <?= ($_POST['aktif']??1)?'checked':'' ?> /> Tampilkan di Website
            </label></div>
        </div>
