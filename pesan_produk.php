...
<body style="display:flex;flex-direction:column;min-height:100vh;">
  ...
  <script>
    const variasiData = <?= json_encode(array_map(fn($v)=>['id'=>$v['id'],'nama'=>$v['nama'],'harga'=>$v['harga'],'gambar'=>$v['gambar']??''],$all_variasi)) ?>;
    ...
    if (selVariasi) {
      ...
      function update(){
        ...
        const harga=parseInt(opt.dataset.harga), gambar=opt.dataset.gambar, jumlah=parseInt(qtyInput.value)||1;
        ...
        const img=document.getElementById('sum-img'); if(img&&gambar) img.src=gambar;
      }
      selVariasi.addEventListener('change',update);
      qtyInput.addEventListener('input',update);
      document.getElementById('qty-min').addEventListener('click',()=>{ if(parseInt(qtyInput.value)>1){qtyInput.value--;update();} });
      document.getElementById('qty-plus').addEventListener('click',()=>{ if(parseInt(qtyInput.value)<99){qtyInput.value++;update();} });
      update();
    }
  </script>
</body>
